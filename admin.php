...
if ($authed && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['do'] ?? '') === 'dates') {
        $result = production_dates_save((array)($_POST['dates'] ?? []));
...
<?php else: ?>
  ...
  <section class="mt-8 rounded-3xl border-2 border-ink bg-white p-6">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h2 class="font-display text-2xl font-bold">Production dates</h2>
        <p class="mt-1 max-w-lg text-sm text-cocoa">
          The only days customers can choose. Past dates drop off by themselves.
          <?php if (!production_dates_are_manual()): ?>
            <span class="font-semibold">Right now these are automatic</span> — the next <?= AUTO_DATE_DAYS ?> open days. Save once to take control.
          <?php endif; ?>
        </p>
      </div>
      <?php if (production_dates_are_manual()): ?>
        <form method="post">
          <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
          <input type="hidden" name="code" value="<?= e(ADMIN_CODE) ?>">
          <input type="hidden" name="do" value="auto_dates">
          <button class="rounded-full border-2 border-ink px-4 py-2 text-sm font-semibold hover:bg-greenlt">Back to automatic</button>
        </form>
      <?php endif; ?>
    </div>
    ...
    <form method="post" class="mt-5" data-dates-form>
      ...
      <div data-date-chips class="mt-5 flex flex-wrap gap-2">
      ...
      <p data-no-dates class="mt-5 hidden rounded-2xl border-2 border-jam px-4 py-3 text-sm text-jam">
        No dates left. Saving now closes ordering until you add one.
      </p>

      <button class="mt-5 rounded-full border-2 border-ink bg-green px-6 py-2.5 font-semibold text-white hover:bg-greendk">Save dates</button>
      <p class="mt-2 text-xs text-cocoa">Nothing is saved until you press this.</p>
    </form>

    <script>
    (() => {
      const form  = document.querySelector('[data-dates-form]');
      ...
      form.querySelector('[data-add-date]').addEventListener('click', () => {
        ...
        if (taken) { input.value = ''; return; }

        const chip = document.createElement('span');
        chip.dataset.chip = '';
        chip.className = 'flex items-center gap-2 rounded-full border-2 border-ink bg-greenlt px-4 py-2 text-sm font-semibold';
        chip.innerHTML = `
